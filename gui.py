...
class MainWindow(QMainWindow):
    def __init__(self):
        """
        Этот класс отвечает за работу интерфейса, заполненяет таблицы и объявляет переменные,
        взаимодействует со страницами StackedWidget
        """
        super().__init__()
        self.facade = Facade()
        self.ui = uic.loadUi("main.ui", self)
        self.page = self.ui.stackedWidget_main
        self.page_id = [0]  # сюда записываются индексы доступных пользователю страниц после авторизации, в зависимости от роли пользователя
        self.now_page = 0
        ...
        self.ui.btn_zero.clicked.connect(self.zero_page)
        self.ui.btn_all_clients.clicked.connect(self.page_all_clients)
        self.ui.btn_exit.clicked.connect(lambda: self.exit(False))
        self.ui.btn_first.clicked.connect(self.first_page)
        self.ui.btn_sec.clicked.connect(self.sec_page)
        self.ui.btn_third.clicked.connect(self.third_page)
    ...
    def Counter(self):
        ...
        if self.time == QTime(0, 3, 0):
            self.messagebox = QMessageBox(self)
            self.messagebox.setWindowTitle("Внимание!")
            self.messagebox.setText("До завершения сеанса осталось 3 минуты!")
    ...
    def exit(self, block):
        ...
        :param block: блокировка по завершению отсчёта таймера
        :return:
        """
        self.now_page = 0
        self.page.setCurrentIndex(self.page_id[self.now_page])
    ...
    def page_all_clients(self):
        """
        Выполняет функцию перехода на таблицу со всеми клиентами
    ...
    def toTable(self):
        """
        Обновляет таблицу со списком всех клиентов
        """
    ...
    def updateTableHistory(self):
        """
        Обновляет таблицу с логами рабочих сессий сотрудников
        """
    ...
    def new_service(self):
        """
        Функия добавления новой услуги
        """
    ...
    def delete_service(self):
        """
        Функция удаления выбранной услуги
        """
        ...
        if rowcount == 0:
            ...
            msg.setText("Нет данных для удаления!")
            ...
            msg = QMessageBox(self)
            msg.setIcon(QMessageBox.Warning)
            msg.setText("Не выбрано поле для удаления!")
            msg.setWindowTitle("Ошибка")
            msg.setStandardButtons(QMessageBox.Ok)
            retval = msg.exec_()
    ...
    def get_from_table(self):
        """
        Получает данные из таблицы, для последующей записи в БД
        """
    ...
    def save_service(self):
        """
        Сохраняет данные об услугах в БД
        Обновляет таблицу с услугами в интерфейсе
        """
        data = self.get_from_table()
        for string in data:
            if string[1] != '':  # если название услуги есть, то данные будут обновлены
                self.facade.update_service(int(string[0]), string[1], string[2], string[3])
            else:  # если названия услуги нет, то данная строка будет удалена
                self.facade.delete_service(int(string[0]))
        self.updateTableServ()

    def build_combobox_clients(self):
        """
        Функция добавляет список клиентов в ComboBox
        """
    ...
    def build_combobox_services(self):
        """
        Функция добавляет список услуг в ComboBox
        """
    ...
    def add_new_request(self):
        """
        Функция оформления нового заказа и отображение в ListWidget
        """
    ...
    def add_service_to_request(self):
        """
        Функция добавления услуги в заказ
        """
        self.service = QListWidgetItem(self.comboBox_serv.currentText())
        self.add_new_field.addItem(self.service)
    ...
    def save_request(self):
        """
        Функция сохраняет новый заказ в БД
        :return:
        """
    ...
    def generateCode(self):
        """
        Функция создания штрих-кода по номеру, дате и времени заказа
        Штрих-код создается в форматах pdf и png
        """
        rv = BytesIO()
    ...
    def mes_box(self, text):
        """
        Открывает messagebox в случае успешного создании штрих-кода.
        :param text: содержит текст для вывода в messagebox
        """
        self.messagebox = QMessageBox(self)
    ...
    def sec_page(self):
        ...
        :return:
        """
        role = self.parent().facade.get_for_authorization(auth_log)
    ...
    def third_page(self):
        ...
        role = self.parent().facade.get_for_authorization(auth_log)
        if role == 'Старший смены' or role == 'Продавец':
            self.btn_third.setEnabled(False)
        else:
            self.now_page = 3
            self.page.setCurrentIndex(self.page_id[self.now_page])
    ...
    def oped_new_client(self):
        """
        Создает и отображает модальное окно создания нового клиента.
        """
...
class DialogAuth(QDialog):
    def __init__(self, parent=None):
        """
        Функция выполняет подключение к кнопкам, создает сцены и объявляет переменные в рамках «graphicsView»
        """
